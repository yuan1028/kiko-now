...
```
5. committed值更新的时候，表示当前有新的日志可以提交给state machine来执行了。(这边交给state machine的过程实际上是通过发消息中带过去的，见 [Raft在etcd中的实现（二）节点发送消息和接收消息流程]({{ site.baseurl }}/etcd-raft-3)，其中在发送消息时会判断是否有可以执行的日志条目)
